...
queue_info_t		queue_info;//需要拨测任务队列
threadpool_t 		tp;
int	raw_fd;
int primary_flag;//是否主机标志
pthread_mutex_t	client_lock;
bool			client_connecting_flag = false;
bool			registered_flag = false;
//dial_srv_cfg_t srv_config;
dial_cfg_t g_cfg;
bool s_debug_switch;
...
{     
		while(thread_exit_flag) 
		{
				...
				{
						pthread_cond_signal(&tp.cond);
						cfg_debug_printf(LOG_LEVEL_BASIC,"queue have %d task,a signal to work thread!!\n",queue_info.cnt);
				}
				else
				{
						sleep(1);
						cfg_debug_printf(LOG_LEVEL_BASIC,"no task queue is empty!!\n");
				}     
		}
}
